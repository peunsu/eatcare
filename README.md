...
본 장 전체에서 아래 세 사전(함수·데이터·SQL 파라미터)의 이름을 그대로 사용한다.
...
| 통계(차트) | 정의/수식 | 비고 |
|---|---|---|
| **영양소 평균 섭취량** (막대) | 영양소 n에 대해 `Σ_{c∈G} avg_map[n][c]` ÷ `|G|` (회원별 기간평균의 그룹 평균) | 분모=그룹 전체 인원(섭취 0 포함) |
